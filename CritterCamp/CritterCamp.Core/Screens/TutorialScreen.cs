...
namespace CritterCamp.Core.Screens {
    class TutorialScreen : MenuScreen {
        ...
        public override void Draw(GameTime gameTime) {
            ...
            sd.DrawString(ScreenManager.Fonts["gillsans"], text, new Vector2(Constants.BUFFER_WIDTH / 2, Constants.BUFFER_HEIGHT - 150), Color.White);
            if (!single) { // it not single player, draw the timer information
                sd.DrawString(ScreenManager.Fonts["gillsans"], "Continuing in ", new Vector2(Constants.BUFFER_WIDTH / 2 - 30, Constants.BUFFER_HEIGHT - 225), Color.White);
                sd.DrawString(ScreenManager.Fonts["gillsans"], timeLeft.ToString(), new Vector2(Constants.BUFFER_WIDTH / 2 + 170, Constants.BUFFER_HEIGHT - 225), Color.White, false, true);
            }
            sd.End();
        }
    }
